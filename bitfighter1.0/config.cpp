...
namespace bitfighter {
	...
	const char *MASTER_SERVER_LIST_ADDRESS( void ) {
		...
		return "IP:199.192.229.168:25955,bitfighter.org:25955";
	}
	...
	extern std::string lcase(std::string strToConvert);

	// Sorts alphanumerically
	extern Sint32 QSORT_CALLBACK alphaSort(std::string *a, std::string *b);
	...
	static void loadForeignServerInfo(CIniFile *ini, IniSettings *iniSettings)
	{
	   ...
	   parsestd::string(ini->GetValue("Connections", "AlwaysPingList", "IP:Broadcast:28000"), iniSettings->alwaysPingList, ',');
	   ...
	   //parsestd::string(ini->GetValue("ForeignServers", "ForeignServerList"), prevServerListFromMaster, ',');
	...
	static void writeLoadoutPresets(CIniFile *ini, GameSettings *settings)
	{
	   ...
	   for(Sint32 i = 0; i < LOADOUT_PRESETS; i++)
	   {
		  if(!settings->getLoadoutPreset(i, preset))
			 continue;

		  std::string presetStr = Ship::loadoutTostd::string(preset);
	...
	static void writePluginBindings(CIniFile *ini)
	{
	   ...
	   if(ini->numSectionComments(section) == 0)
	   {
		  ...
		  addComment(" Plugin1=Key1|ScriptName.lua|Script help std::string");
		  addComment(" ... etc ...");
		  addComment(" The names of the presets are not important, and can be changed. Key combos follow the general form of");
		  addComment(" Ctrl+Alt+Shift+Meta+Super+key (omit unneeded modifiers, you can get correct Input std::strings from the ");
	...
	static void writeConnectionsInfo(CIniFile *ini, IniSettings *iniSettings)
	{
	   ...
	   ini->SetValue(section, "AlwaysPingList", listTostd::string(iniSettings->alwaysPingList, ','));
	}
	 ...
	 void loadLevels(CIniFile *ini, IniSettings *iniSettings)
	{
	   ...
	   {
		  Sint32 numLevels = ini->GetNumEntries("Levels");
		  Vector<std::string> levelValNames;

		  for(Sint32 i = 0; i < numLevels; i++)
		  ...
		  for(Sint32 i = 0; i < numLevels; i++)
		  {
			 ...
			 if (level != "")
				iniSettings->levelList.push_back(std::stringTableEntry(level.c_str()));
		  }
	   }
	}
	...
	// Convert a std::string value to a DisplayMode enum value
	static DisplayMode std::stringToDisplayMode(std::string mode)
	{
	...
	// Convert a std::string value to our sfxSets enum
	static std::string displayModeTostd::string(DisplayMode mode)
	{
	...
	static void loadGeneralSettings(CIniFile *ini, IniSettings *iniSettings)
	{
	   ...
	   iniSettings->displayMode = std::stringToDisplayMode( ini->GetValue(section, "WindowMode", displayModeTostd::string(iniSettings->displayMode)));
	   ...
	   iniSettings->winSizeFact = (float) ini->GetValueF(section, "WindowScalingFactor", iniSettings->winSizeFact);
	   ...
	   Sint32 fps = ini->GetValueI(section, "MaxFPS", iniSettings->maxFPS);
	   if(fps >= 1) 
		  iniSettings->maxFPS = fps;   // Otherwise, leave it at the default value
	   // else warn?

	#ifndef ZAP_DEDICATED
	   gDefaultLineWidth = (float) ini->GetValueF(section, "LineWidth", 2);
	...
	{
	   std::string section = "Diagnostics";
	...
	static void loadTestSettings(CIniFile *ini, IniSettings *iniSettings)
	{
	   ...
	   iniSettings->wallFillColor.set(ini->GetValue("Testing", "WallFillColor", iniSettings->wallFillColor.toRGBstd::string()));
	   iniSettings->wallOutlineColor.set(ini->GetValue("Testing", "WallOutlineColor", iniSettings->wallOutlineColor.toRGBstd::string()));
	...
	{
	   Vector<std::string> rawPresets(LOADOUT_PRESETS);
	   ...
	   for(Sint32 i = 0; i < LOADOUT_PRESETS; i++)
	   {
		  ...
		  if(Ship::std::stringToLoadout(rawPresets[i], loadout))
	...
	static void loadPluginBindings(CIniFile *ini, IniSettings *iniSettings)
	{
	   ...
	   Vector<std::string> words;      // Reusable container

	   ini->GetAllValues("EditorPlugins", values);

	   // Parse the retrieved std::strings.  They'll be in the form "Key Script Help"
	   for(Sint32 i = 0; i < values.size(); i++)
	   {
		  parsestd::string(trim(values[i]), words, '|');
	...
	// Convert a std::string value to our sfxSets enum
	static sfxSets std::stringToSFXSet(std::string sfxSet)
	{
	   return (lcase(sfxSet) == "classic") ? sfxClassicSet : sfxModernSet;
	}


	static float checkVol(float vol)
	{
	...
	{
	   iniSettings->sfxVolLevel       = (float) ini->GetValueI("Sounds", "EffectsVolume",   (Sint32) (iniSettings->sfxVolLevel       * 10)) / 10.0f;
	   iniSettings->musicVolLevel     = (float) ini->GetValueI("Sounds", "MusicVolume",     (Sint32) (iniSettings->musicVolLevel     * 10)) / 10.0f;
	   iniSettings->voiceChatVolLevel = (float) ini->GetValueI("Sounds", "VoiceChatVolume", (Sint32) (iniSettings->voiceChatVolLevel * 10)) / 10.0f;

	   std::string sfxSet = ini->GetValue("Sounds", "SFXSet", "Modern");
	   iniSettings->sfxSet = std::stringToSFXSet(sfxSet);
	...
	static void loadHostConfiguration(CIniFile *ini, IniSettings *iniSettings)
	{
	   ...
	   iniSettings->alertsVolLevel = (float) ini->GetValueI(section, "AlertsVolume", (Sint32) (iniSettings->alertsVolLevel * 10)) / 10.0f;
	   ...
	   Sint32 fps = ini->GetValueI(section, "MaxFPS", iniSettings->maxDedicatedFPS);
	   if(fps >= 1) 
	   ...
	   iniSettings->randomLevels           = (Uint32) ini->GetValueYN(section, "RandomLevels", Sint32(iniSettings->randomLevels) );
	   iniSettings->skipUploads            = (Uint32) ini->GetValueYN(section, "SkipUploads", Sint32(iniSettings->skipUploads) );

	   iniSettings->allowMapUpload         = (Uint32) ini->GetValueYN(section, "AllowMapUpload", Sint32(iniSettings->allowMapUpload) );
	   iniSettings->allowAdminMapUpload    = (Uint32) ini->GetValueYN(section, "AllowAdminMapUpload", Sint32(iniSettings->allowAdminMapUpload) );

	   iniSettings->voteEnable             = (Uint32) ini->GetValueYN(section, "VoteEnable", Sint32(iniSettings->voteEnable) );
	   iniSettings->voteLength             = (Uint32) ini->GetValueI (section, "VoteLength", Sint32(iniSettings->voteLength) );
	   iniSettings->voteLengthToChangeTeam = (Uint32) ini->GetValueI (section, "VoteLengthToChangeTeam", Sint32(iniSettings->voteLengthToChangeTeam) );
	   iniSettings->voteRetryLength        = (Uint32) ini->GetValueI (section, "VoteRetryLength", Sint32(iniSettings->voteRetryLength) );
	   ...
	   parsestd::string(ini->GetValue(section, "MySqlStatsDatabaseCredentials"), args, ',');
	...
	static InputCode getInputCode(CIniFile *ini, const std::string &section, const std::string &key, InputCode defaultValue)
	{
	   const char *code = InputCodeManager::inputCodeTostd::string(defaultValue);
	   return InputCodeManager::std::stringToInputCode(ini->GetValue(section, key, code).c_str());
	}
	...
	{                                
	   std::string section = "KeyboardKeyBindings";
	...
	#define SAVE_BINDING(binding) ini->SetValue(section, InputCodeManager::getBindingName(binding), \
											   InputCodeManager::inputCodeTostd::string(inputCodeManager->getBinding(binding,  mode)));


	static void writeKeyBindings(CIniFile *ini, InputCodeManager *inputCodeManager, const std::string &section, InputMode mode)
	{
	   // Top line evaluates to:
	   // ini->SetValue(section, "SelWeapon1", InputCodeManager::inputCodeTostd::string(inputCodeManager->getBinding(InputCodeManager::BINDING_SELWEAP1,  mode)));
	...
	*/
	static void loadQuickChatMessages(CIniFile *ini)
	{
	   ...
	   Sint32 keys = ini->GetNumSections();
	   ...
	   {
		  std::string keyName = ini->getSectionName(i);
	   ...
	   for(Sint32 i = messages.size()-1; i >= 0; i--)
	   {
		  QuickChatNode node;
		  node.depth = 1;   // This is a top-level message node
		  node.inputCode =  InputCodeManager::std::stringToInputCode(ini->GetValue(messages[i], "Key", "A").c_str());
		  node.buttonCode = InputCodeManager::std::stringToInputCode(ini->GetValue(messages[i], "Button", "Button 1").c_str());
		  std::string str1 = lcase(ini->GetValue(messages[i], "MessageType", "Team"));      // lcase for case insensitivity
	   ...
	   {
		  std::string keyName = ini->getSectionName(i);
		  if(keyName.substr(0, 22) == "QuickChatMessagesGroup" && keyName.find("_") == std::string::npos)   // Found message group
	   ...
	   for(Sint32 i = groups.size()-1; i >= 0; i--)
	   {
		  Vector<std::string> messages;
		  for(Sint32 j = 0; j < keys; j++)
		  {
			 std::string keyName = ini->getSectionName(j);
		  ...
		  node.inputCode =  InputCodeManager::std::stringToInputCode(ini->GetValue(groups[i], "Key", "A").c_str());
		  node.buttonCode = InputCodeManager::std::stringToInputCode(ini->GetValue(groups[i], "Button", "Button 1").c_str());
		  std::string str1 = lcase(ini->GetValue(groups[i], "MessageType", "Team"));      // lcase for case insensitivity
		  node.teamOnly = str1 == "team";
		  node.commandOnly = str1 == "command";
		  ...
		  for(Sint32 j = messages.size()-1; j >= 0; j--)
		  {
			 node.depth = 2;   // This is a message node
			 node.inputCode =  InputCodeManager::std::stringToInputCode(ini->GetValue(messages[j], "Key", "A").c_str());
			 node.buttonCode = InputCodeManager::std::stringToInputCode(ini->GetValue(messages[j], "Button", "Button 1").c_str());
	...
	static void writeDefaultQuickChatMessages(CIniFile *ini, IniSettings *iniSettings)
	{
	   ...
	   {
		  std::string keyName = ini->getSectionName(i);
		  if(keyName.substr(0, 22) == "QuickChatMessagesGroup" && keyName.find("_") == std::string::npos)
			 return;
	   }
	   ...
	   ini->SetValue("QuickChatMessagesGroup1", "Key", InputCodeManager::inputCodeTostd::string(KEY_G));
	   ini->SetValue("QuickChatMessagesGroup1", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_6));
	   ini->SetValue("QuickChatMessagesGroup1", "Caption", "Global");
	   ini->SetValue("QuickChatMessagesGroup1", "MessageType", "Global");

		  ini->SetValue("QuickChatMessagesGroup1_Message1", "Key", InputCodeManager::inputCodeTostd::string(KEY_A));
		  ini->SetValue("QuickChatMessagesGroup1_Message1", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_1));
		  ...
		  ini->SetValue("QuickChatMessagesGroup1_Message2", "Key", InputCodeManager::inputCodeTostd::string(KEY_T));
		  ini->SetValue("QuickChatMessagesGroup1_Message2", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_2));
		  ...
		  ini->SetValue("QuickChatMessagesGroup1_Message3", "Key", InputCodeManager::inputCodeTostd::string(KEY_X));
		  ini->SetValue("QuickChatMessagesGroup1_Message3", "Button", InputCodeManager::inputCodeTostd::string(KEY_UNKNOWN));
		  ...
		  ini->SetValue("QuickChatMessagesGroup1_Message4", "Key", InputCodeManager::inputCodeTostd::string(KEY_E));
		  ini->SetValue("QuickChatMessagesGroup1_Message4", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_3));
		  ...
		  ini->SetValue("QuickChatMessagesGroup1_Message5", "Key", InputCodeManager::inputCodeTostd::string(KEY_C));
		  ini->SetValue("QuickChatMessagesGroup1_Message5", "Button", InputCodeManager::inputCodeTostd::string(KEY_UNKNOWN));
		  ...
		  ini->SetValue("QuickChatMessagesGroup1_Message6", "Key", InputCodeManager::inputCodeTostd::string(KEY_D));
		  ini->SetValue("QuickChatMessagesGroup1_Message6", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_4));
		  ...
		  ini->SetValue("QuickChatMessagesGroup1_Message7", "Key", InputCodeManager::inputCodeTostd::string(KEY_S));
		  ini->SetValue("QuickChatMessagesGroup1_Message7", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_5));
		  ...
		  ini->SetValue("QuickChatMessagesGroup1_Message8", "Key", InputCodeManager::inputCodeTostd::string(KEY_Z));
		  ini->SetValue("QuickChatMessagesGroup1_Message8", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_6));
	   ...
	   ini->SetValue("QuickChatMessagesGroup2", "Key", InputCodeManager::inputCodeTostd::string(KEY_D));
	   ini->SetValue("QuickChatMessagesGroup2", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_5));
	   ini->SetValue("QuickChatMessagesGroup2", "MessageType", "Team");
	   ini->SetValue("QuickChatMessagesGroup2", "Caption", "Defense");

		  ini->SetValue("QuickChatMessagesGroup2_Message1", "Key", InputCodeManager::inputCodeTostd::string(KEY_G));
		  ini->SetValue("QuickChatMessagesGroup2_Message1", "Button", InputCodeManager::inputCodeTostd::string(KEY_UNKNOWN));
		  ...
		  ini->SetValue("QuickChatMessagesGroup2_Message2", "Key", InputCodeManager::inputCodeTostd::string(KEY_D));
		  ini->SetValue("QuickChatMessagesGroup2_Message2", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_1));
		  ...
		  ini->SetValue("QuickChatMessagesGroup2_Message3", "Key", InputCodeManager::inputCodeTostd::string(KEY_Q));
		  ini->SetValue("QuickChatMessagesGroup2_Message3", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_2));
		  ...
		  ini->SetValue("QuickChatMessagesGroup2_Message4", "Key", InputCodeManager::inputCodeTostd::string(KEY_C));
		  ini->SetValue("QuickChatMessagesGroup2_Message4", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_3));
		  ...
		  ini->SetValue("QuickChatMessagesGroup2_Message5", "Key", InputCodeManager::inputCodeTostd::string(KEY_T));
		  ini->SetValue("QuickChatMessagesGroup2_Message5", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_4));
		  ...
		  ini->SetValue("QuickChatMessagesGroup2_Message6", "Key", InputCodeManager::inputCodeTostd::string(KEY_N));
		  ini->SetValue("QuickChatMessagesGroup2_Message6", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_5));
		  ...
		  ini->SetValue("QuickChatMessagesGroup2_Message7", "Key", InputCodeManager::inputCodeTostd::string(KEY_E));
		  ini->SetValue("QuickChatMessagesGroup2_Message7", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_6));
		  ...
		  ini->SetValue("QuickChatMessagesGroup2_Message8", "Key", InputCodeManager::inputCodeTostd::string(KEY_A));
		  ini->SetValue("QuickChatMessagesGroup2_Message8", "Button", InputCodeManager::inputCodeTostd::string(KEY_UNKNOWN));
	   ...
	   ini->SetValue("QuickChatMessagesGroup3", "Key", InputCodeManager::inputCodeTostd::string(KEY_F));
	   ini->SetValue("QuickChatMessagesGroup3", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_4));
	   ini->SetValue("QuickChatMessagesGroup3", "MessageType", "Team");
	   ini->SetValue("QuickChatMessagesGroup3", "Caption", "Flag");

		  ini->SetValue("QuickChatMessagesGroup3_Message1", "Key", InputCodeManager::inputCodeTostd::string(KEY_F));
		  ini->SetValue("QuickChatMessagesGroup3_Message1", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_1));
		  ...
		  ini->SetValue("QuickChatMessagesGroup3_Message2", "Key", InputCodeManager::inputCodeTostd::string(KEY_R));
		  ini->SetValue("QuickChatMessagesGroup3_Message2", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_2));
		  ...
		  ini->SetValue("QuickChatMessagesGroup3_Message3", "Key", InputCodeManager::inputCodeTostd::string(KEY_S));
		  ini->SetValue("QuickChatMessagesGroup3_Message3", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_3));
		  ...
		  ini->SetValue("QuickChatMessagesGroup3_Message4", "Key", InputCodeManager::inputCodeTostd::string(KEY_H));
		  ini->SetValue("QuickChatMessagesGroup3_Message4", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_4));
		  ...
		  ini->SetValue("QuickChatMessagesGroup3_Message5", "Key", InputCodeManager::inputCodeTostd::string(KEY_E));
		  ini->SetValue("QuickChatMessagesGroup3_Message5", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_5));
		  ...
		  ini->SetValue("QuickChatMessagesGroup3_Message6", "Key", InputCodeManager::inputCodeTostd::string(KEY_G));
		  ini->SetValue("QuickChatMessagesGroup3_Message6", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_6));
	   ...
	   ini->SetValue("QuickChatMessagesGroup4", "Key", InputCodeManager::inputCodeTostd::string(KEY_S));
	   ini->SetValue("QuickChatMessagesGroup4", "Button", InputCodeManager::inputCodeTostd::string(KEY_UNKNOWN));
	   ...
	   ini->SetValue("QuickChatMessagesGroup4", "Caption", "Incoming Enemies - Direction");
		  ...
		  ini->SetValue("QuickChatMessagesGroup4_Message1", "Key", InputCodeManager::inputCodeTostd::string(KEY_S));
		  ini->SetValue("QuickChatMessagesGroup4_Message1", "Button", InputCodeManager::inputCodeTostd::string(KEY_UNKNOWN));
		  ...
		  ini->SetValue("QuickChatMessagesGroup4_Message2", "Key", InputCodeManager::inputCodeTostd::string(KEY_E));
		  ini->SetValue("QuickChatMessagesGroup4_Message2", "Button", InputCodeManager::inputCodeTostd::string(KEY_UNKNOWN));
		  ...
		  ini->SetValue("QuickChatMessagesGroup4_Message3", "Key", InputCodeManager::inputCodeTostd::string(KEY_W));
		  ini->SetValue("QuickChatMessagesGroup4_Message3", "Button", InputCodeManager::inputCodeTostd::string(KEY_UNKNOWN));
		  ...
		  ini->SetValue("QuickChatMessagesGroup4_Message4", "Key", InputCodeManager::inputCodeTostd::string(KEY_N));
		  ini->SetValue("QuickChatMessagesGroup4_Message4", "Button", InputCodeManager::inputCodeTostd::string(KEY_UNKNOWN));
		  ...
		  ini->SetValue("QuickChatMessagesGroup4_Message5", "Key", InputCodeManager::inputCodeTostd::string(KEY_V));
		  ini->SetValue("QuickChatMessagesGroup4_Message5", "Button", InputCodeManager::inputCodeTostd::string(KEY_UNKNOWN));
	   ...
	   ini->SetValue("QuickChatMessagesGroup5", "Key", InputCodeManager::inputCodeTostd::string(KEY_V));
	   ini->SetValue("QuickChatMessagesGroup5", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_3));
	   ini->SetValue("QuickChatMessagesGroup5", "MessageType", "Team");
	   ini->SetValue("QuickChatMessagesGroup5", "Caption", "Quick");

		  ini->SetValue("QuickChatMessagesGroup5_Message1", "Key", InputCodeManager::inputCodeTostd::string(KEY_J));
		  ini->SetValue("QuickChatMessagesGroup5_Message1", "Button", InputCodeManager::inputCodeTostd::string(KEY_UNKNOWN));
		  ...
		  ini->SetValue("QuickChatMessagesGroup5_Message2", "Key", InputCodeManager::inputCodeTostd::string(KEY_O));
		  ini->SetValue("QuickChatMessagesGroup5_Message2", "Button", InputCodeManager::inputCodeTostd::string(KEY_UNKNOWN));
		  ...
		  ini->SetValue("QuickChatMessagesGroup5_Message3", "Key", InputCodeManager::inputCodeTostd::string(KEY_A));
		  ini->SetValue("QuickChatMessagesGroup5_Message3", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_1));
		  ...
		  ini->SetValue("QuickChatMessagesGroup5_Message4", "Key", InputCodeManager::inputCodeTostd::string(KEY_W));
		  ini->SetValue("QuickChatMessagesGroup5_Message4", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_2));
		  ...
		  ini->SetValue("QuickChatMessagesGroup5_Message5", "Key", InputCodeManager::inputCodeTostd::string(KEY_V));
		  ini->SetValue("QuickChatMessagesGroup5_Message5", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_3));
		  ...
		  ini->SetValue("QuickChatMessagesGroup5_Message6", "Key", InputCodeManager::inputCodeTostd::string(KEY_E));
		  ini->SetValue("QuickChatMessagesGroup5_Message6", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_4));
		  ...
		  ini->SetValue("QuickChatMessagesGroup5_Message7", "Key", InputCodeManager::inputCodeTostd::string(KEY_G));
		  ini->SetValue("QuickChatMessagesGroup5_Message7", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_5));
		  ...
		  ini->SetValue("QuickChatMessagesGroup5_Message8", "Key", InputCodeManager::inputCodeTostd::string(KEY_Z));
		  ini->SetValue("QuickChatMessagesGroup5_Message8", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_6));
	   ...
	   ini->SetValue("QuickChatMessagesGroup6", "Key", InputCodeManager::inputCodeTostd::string(KEY_R));
	   ini->SetValue("QuickChatMessagesGroup6", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_2));
	   ini->SetValue("QuickChatMessagesGroup6", "MessageType", "Team");
	   ini->SetValue("QuickChatMessagesGroup6", "Caption", "Reponses");

		  ini->SetValue("QuickChatMessagesGroup6_Message1", "Key", InputCodeManager::inputCodeTostd::string(KEY_A));
		  ini->SetValue("QuickChatMessagesGroup6_Message1", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_1));
		  ...
		  ini->SetValue("QuickChatMessagesGroup6_Message2", "Key", InputCodeManager::inputCodeTostd::string(KEY_N));
		  ini->SetValue("QuickChatMessagesGroup6_Message2", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_2));
		  ...
		  ini->SetValue("QuickChatMessagesGroup6_Message3", "Key", InputCodeManager::inputCodeTostd::string(KEY_Y));
		  ini->SetValue("QuickChatMessagesGroup6_Message3", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_3));
		  ...
		  ini->SetValue("QuickChatMessagesGroup6_Message4", "Key", InputCodeManager::inputCodeTostd::string(KEY_S));
		  ini->SetValue("QuickChatMessagesGroup6_Message4", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_4));
		  ...
		  ini->SetValue("QuickChatMessagesGroup6_Message5", "Key", InputCodeManager::inputCodeTostd::string(KEY_T));
		  ini->SetValue("QuickChatMessagesGroup6_Message5", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_5));
		  ...
		  ini->SetValue("QuickChatMessagesGroup6_Message6", "Key", InputCodeManager::inputCodeTostd::string(KEY_D));
		  ini->SetValue("QuickChatMessagesGroup6_Message6", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_6));
	   ...
	   ini->SetValue("QuickChatMessagesGroup7", "Key", InputCodeManager::inputCodeTostd::string(KEY_T));
	   ini->SetValue("QuickChatMessagesGroup7", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_1));
	   ini->SetValue("QuickChatMessagesGroup7", "MessageType", "Global");
	   ini->SetValue("QuickChatMessagesGroup7", "Caption", "Taunts");

		  ini->SetValue("QuickChatMessagesGroup7_Message1", "Key", InputCodeManager::inputCodeTostd::string(KEY_R));
		  ini->SetValue("QuickChatMessagesGroup7_Message1", "Button", InputCodeManager::inputCodeTostd::string(KEY_UNKNOWN));
		  ...
		  ini->SetValue("QuickChatMessagesGroup7_Message2", "Key", InputCodeManager::inputCodeTostd::string(KEY_C));
		  ini->SetValue("QuickChatMessagesGroup7_Message2", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_1));
		  ...
		  ini->SetValue("QuickChatMessagesGroup7_Message3", "Key", InputCodeManager::inputCodeTostd::string(KEY_D));
		  ini->SetValue("QuickChatMessagesGroup7_Message3", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_2));
		  ...
		  ini->SetValue("QuickChatMessagesGroup7_Message4", "Key", InputCodeManager::inputCodeTostd::string(KEY_X));
		  ini->SetValue("QuickChatMessagesGroup7_Message4", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_3));
		  ...
		  ini->SetValue("QuickChatMessagesGroup7_Message5", "Key", InputCodeManager::inputCodeTostd::string(KEY_W));
		  ini->SetValue("QuickChatMessagesGroup7_Message5", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_4));
		  ...
		  ini->SetValue("QuickChatMessagesGroup7_Message6", "Key", InputCodeManager::inputCodeTostd::string(KEY_Q));
		  ini->SetValue("QuickChatMessagesGroup7_Message6", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_5));
		  ...
		  ini->SetValue("QuickChatMessagesGroup7_Message7", "Key", InputCodeManager::inputCodeTostd::string(KEY_E));
		  ini->SetValue("QuickChatMessagesGroup7_Message7", "Button", InputCodeManager::inputCodeTostd::string(BUTTON_6));
	...
	//   for(Uint32 i=0; i<MaxJoystickAxes*2; i++)
	//   {
	//      Vector<std::string> buttonList;
	//      parsestd::string(ini->GetValue("Joystick", "Axes" + itos(i), i<8 ? itos(i+16) : ""), buttonList, ',');
	//      gJoystickMapping.axes[i] = 0;
	//      for(Sint32 j=0; j<buttonList.size(); j++)
	//      {
	...
	//   {
	//      Vector<std::string> buttonList;
	//      parsestd::string(ini->GetValue("Joystick", "Button" + itos(i), i<10 ? itos(i) : ""), buttonList, ',');
	...
	//   {
	//      Vector<std::string> buttonList;
	//      parsestd::string(ini->GetValue("Joystick", "Pov" + itos(i), itos(i+10)), buttonList, ',');
	...
	//   ///for(listTostd::string(alwaysPingList, ',')
	//   for(Uint32 i=0; i<MaxJoystickAxes*2; i++)
	//   {
	//      Vector<std::string> buttonList;
	...
	//      }
	//      ini->SetValue("Joystick", "Axes" + itos(i), listTostd::string(buttonList, ','));
	//   }
	//   for(Uint32 i=0; i<32; i++)
	//   {
	//      Vector<std::string> buttonList;
	...
	//            buttonList.push_back(itos(j));
	//      }
	//      ini->SetValue("Joystick", "Button" + itos(i), listTostd::string(buttonList, ','));
	//   }
	//   for(Uint32 i=0; i<4; i++)
	//   {
	//      Vector<std::string> buttonList;
	...
	//            buttonList.push_back(itos(j));
	//      }
	//      ini->SetValue("Joystick", "Pov" + itos(i), listTostd::string(buttonList, ','));
	//   }
	//}
	...
	void writeServerBanList(CIniFile *ini, BanList *banList)
	{
	   ...
	   std::string delim = banList->getDelimiter();
	   std::string wildcard = banList->getWildcard();
	   ...
	   ini->SetAllValues("ServerBanList", "BanItem", banList->banListTostd::string());
	}
	...
	static void writeSounds(CIniFile *ini, IniSettings *iniSettings)
	{
	   ...
	   ini->SetValueI("Sounds", "EffectsVolume", (Sint32) (iniSettings->sfxVolLevel * 10));
	   ini->SetValueI("Sounds", "MusicVolume",   (Sint32) (iniSettings->musicVolLevel * 10));
	   ini->SetValueI("Sounds", "VoiceChatVolume",   (Sint32) (iniSettings->voiceChatVolLevel * 10));
	...
	{
	   ini->SetValue("Settings",  "WindowMode", displayModeTostd::string(iniSettings->displayMode));
	}


	void saveWindowPosition(CIniFile *ini, Sint32 x, Sint32 y)
	{
	...
	static void writeHost(CIniFile *ini, IniSettings *iniSettings)
	{
	   ...
	   ini->SetValueI (section, "AlertsVolume", (Sint32) (iniSettings->alertsVolLevel * 10));
	   ...
	   ini->setValueYN(section, "RandomLevels", Sint32(iniSettings->randomLevels) );
	   ini->setValueYN(section, "SkipUploads", Sint32(iniSettings->skipUploads) );

	   ini->setValueYN(section, "AllowMapUpload", Sint32(iniSettings->allowMapUpload) );
	   ini->setValueYN(section, "AllowAdminMapUpload", Sint32(iniSettings->allowAdminMapUpload) );


	   ini->setValueYN(section, "VoteEnable", Sint32(iniSettings->voteEnable) );
	   ini->SetValueI(section, "VoteLength", Sint32(iniSettings->voteLength) );
	   ini->SetValueI(section, "VoteLengthToChangeTeam", Sint32(iniSettings->voteLengthToChangeTeam) );
	   ini->SetValueI(section, "VoteRetryLength", Sint32(iniSettings->voteRetryLength) );
	...
	static void writeTesting(CIniFile *ini, GameSettings *settings)
	{
	   ...
	   ini->SetValue  ("Testing", "WallFillColor",   settings->getWallFillColor()->toRGBstd::string());
	   ini->SetValue  ("Testing", "WallOutlineColor", iniSettings->wallOutlineColor.toRGBstd::string());
	...
	static void writePasswordSection_helper(CIniFile *ini, std::string section)
	{
	   ini->addSection(section);
	   if (ini->numSectionComments(section) == 0)
	   {
	...
	// Can't be static -- called externally!
	void writeSkipList(CIniFile *ini, const Vector<std::string> *levelSkipList)
	{
	   ...
	   Vector<std::string> normalizedSkipList;

	   for(Sint32 i = 0; i < levelSkipList->size(); i++)
	   {
		  // "Normalize" the name a little before writing it
		  std::string filename = lcase(levelSkipList->get(i));
	...
	// Constructor
	FolderManager::FolderManager(const std::string &levelDir, const std::string &robotDir,     const std::string &sfxDir,    const std::string &musicDir, 
								 const std::string &iniDir,       const std::string &logDir,    const std::string &screenshotDir, const std::string &luaDir,
	...
	static std::string resolutionHelper(const std::string &cmdLineDir, const std::string &rootDataDir, const std::string &subdir)
	{
	...
	void FolderManager::resolveDirs(GameSettings *settings)
	{
	   ...
	   std::string rootDataDir = cmdLineDirs.rootDataDir;
	...
	std::string FolderManager::resolveLevelDir(const std::string &levelDir)    
	{
	   if(levelDir != "")
	   ...
	   {
		  std::string candidate = strictjoindir(rootDataDir, "levels", levelDir);
	...
	{
	   std::string cmdLineLevelDir = settings->getLevelDir(CMD_LINE);

	   std::string resolved = resolveLevelDir(cmdLineLevelDir);
	   ...
	   if(rootDataDir != "")
	   {
		  std::string candidate = strictjoindir(rootDataDir, "levels");    // Try rootDataDir/levels
		  if(fileExists(candidate))   
		  {
			 levelDir = candidate;
			 return;
	   ...
	   std::string iniLevelDir = settings->getLevelDir(INI);
	...
	extern std::string strictjoindir(const std::string &part1, const std::string &part2);
	extern bool fileExists(const std::string &filename);

	static std::string checkName(const std::string &filename, const Vector<std::string> &folders, const char *extensions[])
	{
	   std::string name;
	   if(filename.find('.') != std::string::npos)       // filename has an extension
	   {
		  for(Sint32 i = 0; i < folders.size(); i++)
		  {
	   ...
	   else
	   {
		  ...
		  {
			 for(Sint32 j = 0; j < folders.size(); j++)
			 {
	...
	std::string FolderManager::findLevelFile(const std::string &filename) const
	{
	   return findLevelFile(levelDir, filename);
	}


	std::string FolderManager::findLevelFile(const std::string &leveldir, const std::string &filename) const
	{
	...
	Vector<std::string> FolderManager::getScriptFolderList() const
	{
	   Vector<std::string> folders;
	   folders.push_back(levelDir);
	...
	Vector<std::string> FolderManager::getHelperScriptFolderList() const
	{
	   Vector<std::string> folders;
	   folders.push_back(luaDir);
	   folders.push_back(levelDir);
	...
	Vector<std::string> FolderManager::getPluginFolderList() const
	{
	   Vector<std::string> folders;
	...
	std::string FolderManager::findLevelGenScript(const std::string &filename) const
	{
	...
	std::string FolderManager::findScriptFile(const std::string &filename) const
	{
	...
	std::string FolderManager::findPlugin(const std::string &filename) const
	{
	   const char *extensions[] = { ".lua", "" };
	...
	std::string FolderManager::findBotFile(const std::string &filename) const          
	{
	   Vector<std::string> folders;
	   folders.push_back(robotDir);
