...
namespace bitfighter {
	...
	Window::Window( Application *app, std::string title, int w, int h, Uint32 flags )
		: m_fps( 0 )
		, m_fps2( 0.0f )
		, m_deltaCount( 0 )
		, m_frameCount( 0 )
	...
	Application *Window::getApplication( void )
	{ return this->m_application; }

	SDL_Window *Window::getWindow( void )
	{ return this->m_window; }

	Uint32 Window::getWindowId( void )
	...
	bool Window::handleEvents( SDL_Event *e )
	{
		for(unsigned int i = 0; i < this->m_renderables.size(); i++) {
			...
			case SDL_MOUSEWHEEL:
				this->m_renderables[i]->handleEvent( e );
				break;
			}
		}
		return true;
	...
	// --- OpenGL Window
	GLWindow::GLWindow( Application *app, std::string title, int w, int h )
		: Window( app, title, w, h, SDL_WINDOW_SHOWN|SDL_WINDOW_OPENGL )
	{
	...
	// --- SDL_Renderer Window
	RendererWindow::RendererWindow( Application *app, std::string title, int w, int h, int driver, Uint32 flags )
		: Window( app, title, w, h )
